...
async fn test_real_vault_kubernetes_auth() {
    ...
    let openssl = std::process::Command::new("openssl")
        .args(["genrsa", "-out", priv_path.to_str().unwrap(), "2048"])
        .status()
        .expect("openssl must be available for k8s test key generation");
    assert!(openssl.success(), "openssl genrsa failed");
    ...
    let pub_pem = tokio::fs::read_to_string(&pub_path).await.unwrap();
    let priv_key = tokio::fs::read(&priv_path).await.unwrap();

    let http = reqwest::Client::new();
    let base = format!("http://{host}:{port}");

    let resp = http
        .post(format!("{base}/v1/sys/auth/kubernetes"))
        .header("x-vault-token", ROOT_TOKEN)
    ...
    assert!(
        resp.status().is_success(),
        "enable kubernetes auth failed: {:?}",
        resp.text().await
    );

    let resp = http
        .post(format!("{base}/v1/auth/kubernetes/config"))
        .header("x-vault-token", ROOT_TOKEN)
        .json(&json!({
    ...
    assert!(
        resp.status().is_success(),
        "kubernetes config failed: {:?}",
        resp.text().await
    );

    let resp = http
        .post(format!("{base}/v1/auth/kubernetes/role/testrole"))
        .header("x-vault-token", ROOT_TOKEN)
        .json(&json!({
    ...
    let jwt = jsonwebtoken::encode(
        &header,
        &claims,
        &jsonwebtoken::EncodingKey::from_rsa_pem(&priv_key).unwrap(),
    )
    .unwrap();

    let jwt_file = tmp.path().join("token");
    tokio::fs::write(&jwt_file, jwt).await.unwrap();

    let _env_guard = ENV_LOCK.lock().await;
    unsafe {
        std::env::set_var("VAULTENV_KUBERNETES_JWT_PATH", jwt_file.to_str().unwrap());
    }
